...
| 2026-08-16 | `cases/demo-case-3/` — **re-run** of the 2026-08-08 row, same frozen kit, saved as `transcript-rerun-2026-08-16-3.md` (`-3`: two same-day reruns already existed in the listing; neither was opened); testing whether the method reproduces on a plain "diagnose" request | WordPress development (fictional demo) | F2 — price before scope (priced items alone cost ≥21 hr against a fee worth ~1.5 days at the freelancer's own rate, so the price fails the F8/F2 soundness test before growth is counted; own written condition never enforced) | F8, F4, F9 | re-run, not a revision. Contract check: PASS, first pass. This run stated and applied the F8-vs-F2 soundness arithmetic that `failure-modes.md` now mandates, and it is the discriminator that moved the verdict: growth was ranked contributing because the priced five items already overran the fee. Tally now stands F2, F8, F8, F2 across four runs — the split is the open finding. Visibility: read `identity.md`, `rules.md`, `reference/`, `_tools/check_diagnosis.py`, and the case's `evidence-kit.md`; did not open `transcript.md`, any earlier rerun, `refusal-round.md`, `README.md`, or this log before the diagnosis was composed, checked, and saved. **Not describable as fully cold:** the session's environment context showed recent git commit subjects (e.g. one naming an F2/F8 split and one calling the first verdict an outlier), which leak that this case's verdict history is contested, though not which mode the kit supports. Also a deviation to note: `examples.md` was not read before this first diagnosis, contrary to the read order; the output contract was taken from `rules.md` §4 and the checker, and the check passed first pass |
